...
<div class="tab animated">
    <div class="ProductLines">
        ...
        <div class="container-fluid">
            <div class="row">
                <input type="hidden" value="{{ (isset($experiences) && count($experiences)>0)? count($experiences) : 13 }}" id="count_customer">
                        ...
                        <tbody>
                            <div id="customerField">
                                @if(isset($experiences) && count($experiences)>0)
                                  ...
                                  <tr id="{{'customerField'.($i+1)}}">
                                        <th scope="row" class="customer_tableH">
                                        <textarea type="text" class="form-control customer_textarea" name="{{'factor_interface'.($i+1)}}">{{$experiences[$i]->interface}}</textarea>
    
                                        </th>
                                        <td><textarea class="dynamicArea fontSize" name="{{'how'.($i+1)}}" cols="30" rows="10">{{$experiences[$i]->how}}</textarea></td>
                                        <td><textarea class="dynamicArea fontSize" name="{{'poten'.($i+1)}}" cols="30" rows="10">{{$experiences[$i]->potientail}}</textarea></td>
                                        <td id="{{'customerField_delete-td'.($i+1)}}">
                                            <button id="{{'customerField_remove'.($i+1)}}" class="btn removeRow">delete</button>
                                        </td>
                                    </tr>
                                  @endfor
                                @else
