...
<body>
    ...
    <form id="submitForm">
        @include('welcome')
        @include('intro')
        @include('edit_company_details')
        @include('edit_aduit_tools')
        @include('edit_industry_leadership')
        @include('edit_market_trends')    
        @include('edit_customer_experience')   
        @if(isset($beas) && count($beas)>0)  
        @include('edit_turbulence_impact')
        @else
        @include('turbulence_impact')
        @endif
        @if((isset($sw_products) && count($sw_products)>0) ||(isset($sw_peoples) && count($sw_peoples)>0
            )||(isset($sw_process) && count($sw_process)>0) || (isset($sw_operation) && count($sw_operation)>0)
            ||(isset($sw_organisation) && count($sw_organisation)>0))
        @include('edit_sw')
        @else
        @include('sw')
        @endif
        ...
        @if((isset($ansoffs_pens) && count($ansoffs_pens)>0) ||(isset($ansoffs_devs) && count($ansoffs_devs)>0
            )||(isset($ansoffs_services) && count($ansoffs_services)>0) || (isset($ansoffs_divs) && count($ansoffs_divs)>0))
        @include('edit_anshof')
        @else
        @include('anshof')
        @endif
